...
/**
 * <p>A Bloom filter is a probabilistic data structure which can be sent to another client so
 * that it can avoid
 * sending us transactions that aren't relevant to our set of keys. This allows for significantly
 * more efficient
 * use of available network bandwidth and CPU time.</p>
 * <p/>
 * <p>Because a Bloom filter is probabilistic, it has a configurable false positive rate. So the
 * filter will sometimes
 * match transactions that weren't inserted into it, but it will never fail to match transactions
 * that were. This is
 * a useful privacy feature - if you have spare bandwidth the false positive rate can be
 * increased so the remote peer
...
public class BloomFilter extends Message {
    ...
    // A filter of 20,000 items and a false positive rate of 0.1% or one of 10,
    // 000 items and 0.0001% is just under 36,000 bytes
     ...
     */
    public BloomFilter(byte[] payloadBytes) throws ProtocolException {
     ...
     * rate when the given
     * number of elements have been inserted. If the filter would otherwise be larger than the
     ...
     * controlled by the
     * false positive rate. For reference, as of block 187,000, the total number of addresses
     * used in the chain was
     * roughly 4.5 million. Thus, if you use a false positive rate of 0.001 (0.1%),
     * there will be, on average, 4,500
     * distinct public keys/addresses which will be thought to be yours by nodes which have your
     * bloom filter, but
     * which are not actually yours. Keep in mind that a remote node can do a pretty good job
     * estimating the order of
     * magnitude of the false positive rate of a given filter you provide it when considering the
     * anonymity of a given
     * filter.</p>
     * <p/>
     * <p>In order for filtered block download to function efficiently,
     * the number of matched transactions in any given
     * block should be less than (with some headroom) the maximum size of the MemoryPool used by
     * the Peer
     ...
     * It should be a random value, however secureness of the random value is of no great
     * consequence.</p>
     * <p/>
     * <p>updateFlag is used to control filter behaviour on the server (remote node) side when it
     * encounters a hit.
     * The purpose of this flag is to reduce network round-tripping and avoid over-dirtying the
     * filter for
     * the most common
     * wallet configurations.</p>
     */
    public BloomFilter(int elements, double falsePositiveRate, long randomNonce,
                       BloomUpdate updateFlag) {
        // The following formulas were stolen from Wikipedia's page on Bloom Filters (with the
        // addition of min(..., MAX_...))
        //                        Size required for a given number of elements and false-positive
        // rate
        int size = Math.min((int) (-1 / (Math.pow(Math.log(2),
                2)) * elements * Math.log(falsePositiveRate)), (int) MAX_FILTER_SIZE * 8) / 8;
        data = new byte[size <= 0 ? 1 : size];
        // Optimal number of hash functions for a given filter size and element count.
        hashFuncs = Math.min((int) (data.length * 8 / (double) elements * Math.log(2)),
                MAX_HASH_FUNCS);
    ...
    /**
     * Returns the theoretical false positive rate of this filter if were to contain the given
     * number of elements.
     */
    public double getFalsePositiveRate(int elements) {
        return Math.pow(1 - Math.pow(Math.E, -1.0 * (hashFuncs * elements) / (data.length * 8)),
                hashFuncs);
    }
    ...
    private int hash(int hashNum, byte[] object) {
        // The following is MurmurHash3 (x86_32), see http://code.google
        // .com/p/smhasher/source/browse/trunk/MurmurHash3.cpp
    ...
    /**
     * Returns true if the given object matches the filter either because it was inserted,
     * or because we have a
     * false-positive.
     */
    ...
    /**
     * Sets this filter to match all objects. A Bloom filter which matches everything may seem
     * pointless, however,
     * it is useful in order to reduce steady state bandwidth usage when you want full blocks.
     * Instead of receiving
     * all transaction data twice, you will receive the vast majority of all transactions just
     * once, at broadcast time.
     * Solved blocks will then be send just as Merkle trees of tx hashes,
     * meaning a constant 32 bytes of data for each
    ...
    /**
     * Returns true if this filter will match anything. See {@link net.bither.bitherj.core
