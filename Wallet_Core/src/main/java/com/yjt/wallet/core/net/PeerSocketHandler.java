...
import com.yjt.wallet.core.message.BitcoinSerializer;
import com.yjt.wallet.core.message.Message;
import com.yjt.wallet.core.message.PeerAddress;
 ...
 * {@link net.bither.bitherj.net} classes and {@link net.bither.bitherj.core.Peer}.
 */
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamParser {
    ...
    // If we close() before we know our writeTarget, set this to true to call writeTarget
    // .closeConnection() right away.
    private boolean closePending = false;
    // writeTarget will be thread-safe, and may call into PeerGroup, which calls us,
    // so we should call it unlocked
    @VisibleForTesting
    MessageWriteTarget writeTarget = null;

    // The ByteBuffers passed to us from the writeTarget are static in size,
    // and usually smaller than some messages we
    // will receive. For SPV clients, this should be rare (ie we're mostly dealing with small
    // transactions), but for
    // messages which are larger than the read buffer, we have to keep a temporary buffer with
    // its bytes.
    ...
    public PeerSocketHandler(InetSocketAddress remoteIp) {
        serializer = new BitcoinSerializer();
        this.peerAddress = new PeerAddress(remoteIp);
    }

    public PeerSocketHandler(PeerAddress peerAddress) {
        serializer = new BitcoinSerializer();
    ...
    /**
     * Closes the connection to the peer if one exists, or immediately closes the connection as
     * soon as it opens
     */
    public void close() {
        lock.lock();
        ...
        try {
            ...
            for (int i = 0;
                ...
                if (largeReadBuffer != null) {
                    ...
                    int bytesToGet = Math.min(buff.remaining(),
                            largeReadBuffer.length - largeReadBufferPos);
                ...
                try {
                    ...
                    // If we went through the whole buffer without a full message,
                    // we need to use the largeReadBuffer
                    if (i == 0 && buff.limit() == buff.capacity()) {
                        ...
                        try {
                            ...
                            // Initialize the largeReadBuffer with the next message's size and
                            // fill it with any bytes
                            // left in buff
                            largeReadBuffer = new byte[header.size];
                            largeReadBufferPos = buff.remaining();
                            buff.get(largeReadBuffer, 0, largeReadBufferPos);
                        } catch (BufferUnderflowException e1) {
                            // If we went through a whole buffer's worth of bytes without getting
                            // a header, give up
                            // In cases where the buff is just really small,
                            // we could create a second largeReadBuffer
                        ...
                        // which saves us from skipping messages by
            ...
            exceptionCaught(e);
            return -1; // Returning -1 also throws an IllegalStateException upstream and kills
            // the connection
        }
    }
     ...
     * never be called, it is
     * called automatically by {@link net.bither.bitherj.net.NioClientManager} once the socket finishes initialization.
     */
    @Override
