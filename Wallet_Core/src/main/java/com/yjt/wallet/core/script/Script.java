...
package com.yjt.wallet.core.script;
...
import com.yjt.wallet.core.utils.UnsafeByteArrayOutputStream;
...
public class Script {
     ...
     *
     * @param programBytes
     *         Array of program bytes from a transaction.
     */
    ...
    /**
     * Returns true if this script is of the form <sig> ScriptOpCodes.OP_CHECKSIG. This form was originally intended for transactions
    ...
    public boolean isMultiSigRedeem() {
        boolean result = ScriptOpCodes.OP_1 <= this.chunks.get(0).opcode && this.chunks.get(0).opcode <= ScriptOpCodes.OP_16;
        for (int i = 1; i < this.chunks.size() - 2; i++) {
            result &= this.chunks.get(i).data != null && this.chunks.get(i).data.length > 2;
        }
        result &= ScriptOpCodes.OP_1 <= this.chunks.get(this.chunks.size() - 2).opcode && this.chunks.get(this.chunks.size() - 2).opcode <= ScriptOpCodes.OP_16;
        result &= this.chunks.get(this.chunks.size() - 1).opcode == ScriptOpCodes.OP_CHECKMULTISIG;
        return result;
    }
     ...
     *         if the script is none of the named forms.
     */
    ...
    public List<byte[]> getSigs() {
        ...
        if (chunks.size() == 1 && chunks.get(0).isPushData()) {
            result.add(chunks.get(0).data);
                ...
                && chunks.get(1).data.length > 2) {
            result.add(chunks.get(0).data);
        } else if (chunks.size() >= 3 && chunks.get(0).opcode == ScriptOpCodes.OP_0) {
    ...
    public String getFromAddress() throws ScriptException {
        ...
        } else if (this.chunks.size() >= 3 && this.chunks.get(0).opcode == ScriptOpCodes.OP_0) {
            boolean isPay2SHScript = true;
            for (int i = 1; i < this.chunks.size(); i++) {
                isPay2SHScript &= (this.chunks.get(i).data != null && this.chunks.get(i).data.length > 2);
            }
    ...
    /**
     * Creates a program that requires at least N of the given keys to sign, using ScriptOpCodes.OP_CHECKMULTISIG.
     */
    ...
    static int decodeFromOpN(int opcode) {
        checkArgument((opcode == ScriptOpCodes.OP_0 || opcode == ScriptOpCodes.OP_1NEGATE) || (opcode >= ScriptOpCodes.OP_1 && opcode <= ScriptOpCodes.OP_16), "decodeFromOpN called on non ScriptOpCodes.OP_N opcode");
    ...
    public int getNumberOfBytesRequiredToSpend(@Nullable ECKey pubKey, @Nullable Script redeemScript) {
        if (isPayToScriptHash()) {
            ...
            // scriptSig: ScriptOpCodes.OP_0 <sig> [sig] [sig...]
    ...
    public List<byte[]> getP2SHPubKeys(Tx tx, int index) {
        ...
        if (pubKeyCount < 0 || pubKeyCount > 20) {
            throw new ScriptException("[Script Error] ScriptOpCodes.OP_CHECKMULTISIG(VERIFY) with pubkey count out of range");
        }
        ...
        if (sigCount < 0 || sigCount > pubKeyCount) {
            throw new ScriptException("[Script Error] ScriptOpCodes.OP_CHECKMULTISIG(VERIFY) with sig count out of range");
        }
    ...
    public boolean isSentToMultiSig() {
        ...
        // Must end in ScriptOpCodes.OP_CHECKMULTISIG[VERIFY].
        if (!chunk.isOpCode()) {
            return false;
        }
        ...
        try {
            // Second to last chunk must be an ScriptOpCodes.OP_N opcode and there should be that many data chunks (keys).
            ...
            // First chunk must be an ScriptOpCodes.OP_N opcode too.
            if (decodeFromOpN(chunks.get(0).opcode) < 1) {
                return false;
            }
        } catch (IllegalStateException e) {
            return false;   // Not an ScriptOpCodes.OP_N opcode.
        }
        return true;
    }
        ...
        for (ScriptChunk chunk : script.chunks) {
            ...
            if (!chunk.isOpCode()) {
                ...
                if (opcode == ScriptOpCodes.OP_VERIF || opcode == ScriptOpCodes.OP_VERNOTIF) {
                    throw new ScriptException("Script included ScriptOpCodes.OP_VERIF or ScriptOpCodes.OP_VERNOTIF");
                }
                ...
                switch (opcode) {
                    case ScriptOpCodes.OP_IF:
                        ...
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_IF on an empty stack");
                        }
                    ...
                    case ScriptOpCodes.OP_NOTIF:
                        ...
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_NOTIF on an empty stack");
                        }
                        ifStack.add(!castToBool(stack.pollLast()));
                        continue;
                    case ScriptOpCodes.OP_ELSE:
                        if (ifStack.isEmpty()) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_ELSE without ScriptOpCodes.OP_IF/NOTIF");
                        }
                        ifStack.add(!ifStack.pollLast());
                        continue;
                    case ScriptOpCodes.OP_ENDIF:
                        if (ifStack.isEmpty()) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_ENDIF without ScriptOpCodes.OP_IF/NOTIF");
                        }
                        ifStack.pollLast();
                        continue;
                }
                ...
                switch (opcode) {
                    // ScriptOpCodes.OP_0 is no opcode
                    ...
                    case ScriptOpCodes.OP_VERIFY:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_VERIFY on an empty stack");
                        }
                        if (!castToBool(stack.pollLast())) {
                            throw new ScriptException("ScriptOpCodes.OP_VERIFY failed");
                        }
                        break;
                    case ScriptOpCodes.OP_RETURN:
                        throw new ScriptException("Script called ScriptOpCodes.OP_RETURN");
                    case ScriptOpCodes.OP_TOALTSTACK:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_TOALTSTACK on an empty stack");
                        }
                        altstack.add(stack.pollLast());
                        break;
                    case ScriptOpCodes.OP_FROMALTSTACK:
                        if (altstack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_TOALTSTACK on an empty altstack");
                        }
                        stack.add(altstack.pollLast());
                        break;
                    case ScriptOpCodes.OP_2DROP:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_2DROP on a stack with size < 2");
                        }
                        stack.pollLast();
                        stack.pollLast();
                        break;
                    case ScriptOpCodes.OP_2DUP:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_2DUP on a stack with size < 2");
                        }
                    ...
                    case ScriptOpCodes.OP_3DUP:
                        if (stack.size() < 3) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_3DUP on a stack with size < 3");
                        }
                    ...
                    case ScriptOpCodes.OP_2OVER:
                        if (stack.size() < 4) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_2OVER on a stack with size < 4");
                        }
                    ...
                    case ScriptOpCodes.OP_2ROT:
                        if (stack.size() < 6) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_2ROT on a stack with size < 6");
                        }
                    ...
                    case ScriptOpCodes.OP_2SWAP:
                        if (stack.size() < 4) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_2SWAP on a stack with size < 4");
                        }
                    ...
                    case ScriptOpCodes.OP_IFDUP:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_IFDUP on an empty stack");
                        }
                    ...
                    case ScriptOpCodes.OP_DROP:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_DROP on an empty stack");
                        }
                        stack.pollLast();
                        break;
                    case ScriptOpCodes.OP_DUP:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_DUP on an empty stack");
                        }
                        stack.add(stack.getLast());
                        break;
                    case ScriptOpCodes.OP_NIP:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_NIP on a stack with size < 2");
                        }
                    ...
                    case ScriptOpCodes.OP_OVER:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_OVER on a stack with size < 2");
                        }
                    ...
                    case ScriptOpCodes.OP_ROLL:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_PICK/ScriptOpCodes.OP_ROLL on an empty stack");
                        }
                        long val = castToBigInteger(stack.pollLast()).longValue();
                        if (val < 0 || val >= stack.size()) {
                            throw new ScriptException("ScriptOpCodes.OP_PICK/ScriptOpCodes.OP_ROLL attempted to get data deeper than stack size");
                        }
                    ...
                    case ScriptOpCodes.OP_ROT:
                        if (stack.size() < 3) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_ROT on a stack with size < 3");
                        }
                    ...
                    case ScriptOpCodes.OP_TUCK:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_SWAP on a stack with size < 2");
                        }
                    ...
                    case ScriptOpCodes.OP_SIZE:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_SIZE on an empty stack");
                        }
                    ...
                    case ScriptOpCodes.OP_EQUAL:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_EQUALVERIFY on a stack with size < 2");
                        }
                    ...
                    case ScriptOpCodes.OP_EQUALVERIFY:
                        ...
                        if (!Arrays.equals(stack.pollLast(), stack.pollLast())) {
                            throw new ScriptException("ScriptOpCodes.OP_EQUALVERIFY: non-equal data");
                        }
                        break;
                    ...
                    case ScriptOpCodes.OP_NUMEQUALVERIFY:
                        if (stack.size() < 2) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_NUMEQUALVERIFY on a stack with size < 2");
                        }
                        ...
                        if (!OPNUMEQUALVERIFYnum1.equals(OPNUMEQUALVERIFYnum2)) {
                            throw new ScriptException("ScriptOpCodes.OP_NUMEQUALVERIFY failed");
                        }
                        break;
                    ...
                    case ScriptOpCodes.OP_RIPEMD160:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_RIPEMD160 on an empty stack");
                        }
                    ...
                    case ScriptOpCodes.OP_SHA1:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_SHA1 on an empty stack");
                        }
                        try {
                    ...
                    case ScriptOpCodes.OP_SHA256:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_SHA256 on an empty stack");
                        }
                        try {
                    ...
                    case ScriptOpCodes.OP_HASH160:
                        if (stack.size() < 1) {
                            throw new ScriptException("Attempted ScriptOpCodes.OP_HASH160 on an empty stack");
                        }
     ...
     * @param scriptSigIndex
     *         The index in txContainingThis of the scriptSig (note: NOT the index of the scriptPubKey).
     * @param scriptPubKey
     *         The connected scriptPubKey containing the conditions needed to claim the value.
     * @param enforceP2SH
     *         Whether "pay to script hash" rules should be enforced. If in doubt, set to true.
     */
